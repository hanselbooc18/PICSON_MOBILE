...
export default function AdminPatientProfileScreen() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setError("Unable to load patient record. Please try again.");
        console.error(err);
      } finally {
        setLoading(false);
      }
  ...
  const renderRecordItem = (item: Record<string, unknown>) => {
    const createdAt = item.created_at || item.updated_at || item.id || "-";
    return (
      <View style={styles.recordRow}>
        <Text style={styles.recordTitle}>{String(createdAt)}</Text>
        <Text style={styles.recordText}>{JSON.stringify(item)}</Text>
      </View>
    );
  };
  ...
  const renderSection = (title: string, items: Record<string, unknown>[]) => (
    <View style={styles.section}>
      <View style={styles.sectionHeader}>
        ...
        <Text style={styles.sectionMeta}>{items.length} record{items.length === 1 ? "" : "s"}</Text>
      </View>
      {items.length === 0 ? (
